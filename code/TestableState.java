...
public class TestableState {
    ...
    public TestableState(State state) {
        lost = state.hasLost();
        ...
        for (int i = 0; i < state.getField().length; i++) {
            for (int j = 0; j < state.getField()[i].length; j++) {
                field[i][j] = state.getField()[i][j];
            }
        }

        for (int i = 0; i < state.getTop().length; i++) {
            top[i] = state.getTop()[i];
        }
    }

    public TestableState clone() {
        TestableState state = new TestableState();

        state.lost = lost;
        state.turn = turn;
        state.cleared = cleared;
        state.nextPiece = nextPiece;
        state.field = new int[ROWS][COLS];
        state.top = new int[COLS];

        for (int i = 0; i < field.length; i++) {
            for (int j = 0; j < field[i].length; j++) {
                state.field[i][j] = field[i][j];
            }
        }
	...
	public TestableState testMove(int move) {
		return testMove(legalMoves[nextPiece][move]);
	}
	
	// test a move based on an array of orient and slot
	public TestableState testMove(int[] move) {
		return testMove(move[ORIENT],move[SLOT]);
	}

    // test a move
    public TestableState testMove(int orient, int slot) {
        TestableState state = clone();
